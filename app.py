...
#
# Технический момент: st.chat_input обязан находиться на "верхнем уровне"
# страницы, а не внутри st.tabs — иначе он не прилипает к низу экрана.
# Поэтому переключение видов сделано через обычные кнопки в сайдбаре
# и session_state, а не через вкладки.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# БАННЕР С НАПОМИНАНИЯМИ — считается заново при каждом открытии приложения
# ---------------------------------------------------------------------------

due_reminders = db.compute_due_reminders(user_id, date.today())
if due_reminders:
    lines = []
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Думаю…"):
                ...
                open_reminders = db.get_active_reminders(user_id)
                ...
                if intent == "add_task":
                    ...
                    for u in interpretation.get("updates", []):
                        ...
                        if task_id and changes:
                            ...
                            update_results.append({"task_id": task_id, "changes": changes})
                    ...
                    reminder_results = []
                    for r in interpretation.get("reminders", []):
                        created = db.add_reminder(
                            user_id=user_id,
                            title=r.get("title", "").strip(),
                            event_date=r.get("event_date"),
                            event_time=r.get("event_time"),
                            recurrence=r.get("recurrence", "once"),
                            lead_days=r.get("lead_days", [1]),
                            remind_on_day=r.get("remind_on_day", True),
                            repeat_on_day=r.get("repeat_on_day", False),
                        )
                        reminder_results.append(created)

                elif intent == "confirm_reminder":
                    confirmation_results = []
                    for c in interpretation.get("confirmations", []):
                        reminder_id = c.get("reminder_id")
                        if reminder_id:
                            db.confirm_reminder(user_id, reminder_id, date.today().year)
                            confirmation_results.append({"reminder_id": reminder_id})
                    ...
                    for la in interpretation.get("list_actions", []):
                        ...
                        if action == "add" and item_text:
                            ...
                            # находим подходящий пункт по смыслу среди пунктов этого списка
        ...
        if intent in ("add_task", "list_action", "delete_task"):
            # новая категория или список могли появиться — обновляем сайдбар
            st.rerun()


# ---------------------------------------------------------------------------
# ФУНКЦИЯ ОТРИСОВКИ "ЕЖЕДНЕВНИКА" — переиспользуется для полного вида
# и для вида, отфильтрованного по категории
# ---------------------------------------------------------------------------

def _format_time_range(due_time, duration_minutes):
    """Превращает '18:00:00' + 60 мин в '18:00 - 19:00'. Без времени — прочерк."""
    if not due_time:
        return "—"
...
def _mark_done(task_id):
    db.update_task(user_id, task_id, status="готово")
...
def render_planner(tasks_unfiltered, key_prefix, default_category="", reminders=None):
        ...
        for due_date in sorted(by_date.keys()):
            ...
            for t in by_date[due_date]["tasks"]:
                ...
                cols[0].checkbox("", value=(t["status"] == "готово"), key=f"{key_prefix}_d_{t['id']}",
                                  on_change=(lambda tid=t["id"], s=t["status"]:
                                             _mark_undone(tid) if s == "готово" else _mark_done(tid)))
                cols[1].write(_format_time_range(t.get("due_time"), t.get("duration_minutes")))
                task_text = t["task"]
                if t["status"] == "готово":
    ...
    with st.expander("➕ Добавить задачу вручную"):
        with st.form(f"{key_prefix}_manual_add_form", clear_on_submit=True):
            ...
            m_time = m_col2.time_input("Время (необязательно)", value=None)
            m_duration = m_col3.number_input("Длительность, мин", min_value=5, value=60, step=5)
            m_priority = m_col4.selectbox("Приоритет", ["высокий", "средний", "низкий"], index=1)
            m_category = st.text_input("Категория (необязательно)", value=default_category)
            if st.form_submit_button("Добавить") and m_task.strip():
                db.add_task(
                    user_id=user_id,
                    task=m_task.strip(),
                    due_date=m_date.isoformat() if m_date else None,
                    due_time=m_time.strftime("%H:%M") if m_time else None,
    ...
    export_df = pd.DataFrame(tasks_unfiltered)
    ...
    with st.expander("🛠️ Расширенное редактирование (вся таблица целиком)"):
        ...
        raw_df = pd.DataFrame(tasks_unfiltered)
        if not raw_df.empty:
            column_order = ["id", "task", "due_date", "due_time", "duration_minutes", "priority",
                             "category", "status", "due_date_confidence", "source", "created_at"]
...
# ---------------------------------------------------------------------------
# ВИД: КАТЕГОРИЯ (ежедневник, отфильтрованный по одной категории)
# ---------------------------------------------------------------------------

elif st.session_state.view == "category":
    cat = st.session_state.get("active_category", "")
    st.subheader(f"🏷️ {cat}")
    render_planner(db.get_tasks_by_category(user_id, cat), key_prefix=f"cat_{cat}", default_category=cat)


# ---------------------------------------------------------------------------
# ВИД: СПИСОК (покупки, дни рождения, вещи в поездку и т.п. — НЕ задачи)
# ---------------------------------------------------------------------------
